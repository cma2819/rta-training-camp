...
export default async ({
  ...
  const generateHtmlFiles = async () => {
    ...
    await Promise.all([
      ...graphicsInputs.map(async (input) => { await generateHtml(input, graphicsTemplateHtml, graphicsOutdir); },
      ),
      ...dashboardInputs.map(async (input) => { await generateHtml(input, dashboardTemplateHtml, dashboardOutdir); },
      ),
    ]);
  };
